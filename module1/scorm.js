// ─────────────────────────────────────────────────────────────────
//  SCORM runtime adapter — VCPL C1/M1 Password Hygiene
...
// ── Per-question logging (cmi.interactions) ──────────────────────
// letterFor(i) → "a".."d"; SCORM choice responses are single letters.
function scormLogInteraction(question, chosenIndex, correctIndex, isCorrect) {
  if (!initialised) return;
  const n = interactionN++;
  const p = "cmi.interactions." + n + ".";
  const letter = i => String.fromCharCode(97 + i);

  set(p + "id",   question.id);
  set(p + "type", "choice");
  set(p + "correct_responses.0.pattern", letter(correctIndex));
  set(p + "weighting", "1");

  if (IS_2004) {
    set(p + "description",     question.text.substring(0, 250));
    set(p + "learner_response", letter(chosenIndex));
    set(p + "result",           isCorrect ? "correct" : "incorrect");
    set(p + "objectives.0.id",  question.lo);
  } else {
    set(p + "student_response", letter(chosenIndex));
    set(p + "result",           isCorrect ? "correct" : "wrong");
    set(p + "objectives.0.id",  question.lo);
    set(p + "time",             new Date().toTimeString().substring(0, 8));
  }
  commit();
}

// ── Attempt result ───────────────────────────────────────────────
// scaled 0..1 · passed when scaled >= 0.70 · completion needs video + quiz
function scormSubmitResult(correctCount, totalCount, videoWatched) {
  if (!initialised) return;
  const scaled  = totalCount ? correctCount / totalCount : 0;
  const raw     = Math.round(scaled * 100);
  const passed  = scaled >= 0.70;

  if (IS_2004) {
    set("cmi.score.raw",    raw);
    set("cmi.score.min",    "0");
    set("cmi.score.max",    "100");
    set("cmi.score.scaled", scaled.toFixed(2));
    set("cmi.success_status", passed ? "passed" : "failed");
    set("cmi.completion_status", (passed && videoWatched) ? "completed" : "incomplete");
    set("cmi.session_time", iso8601Time(elapsedSeconds()));
    set("cmi.exit", (passed && videoWatched) ? "normal" : "suspend");
  } else {
    set("cmi.core.score.raw", raw);
    set("cmi.core.score.min", "0");
    set("cmi.core.score.max", "100");
    // SCORM 1.2 has one status field: passed/failed carries both meanings.
    set("cmi.core.lesson_status", (passed && videoWatched) ? "passed" : "failed");
    set("cmi.core.session_time",  scormTime(elapsedSeconds()));
    set("cmi.core.exit", (passed && videoWatched) ? "normal" : "suspend");
  }
  commit();
}
